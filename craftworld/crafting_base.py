...
TASKS = [eval_makebread, eval_choptree, eval_buildhouse, eval_choprock]
...
class CraftingBase(Env):
    ...
    def __init__(self, size=[10, 10], res=3, add_objects=[], visible_agent=True, state_obs=False, state_fusion=False,
                 ...
                 use_exit=False, success_function=None, pretty_renderable=False, fixed_init_state=False, task_id=None,
                 step_cost=1.0,
        ...
        if mode == 'human':
            ...
            for obj in to_get_obs:
                ...
                if root is not None:
                    if root in self.SPRITES:
                        row, col = self.state['object_positions'][obj]
                        img[row * self.renderres:(row + 1) * self.renderres,
                        col * self.renderres:(col + 1) * self.renderres, :] += self.BIGSPRITES[root]

            if self.visible_agent:
                row, col = self.state[AGENT]
                img[row * self.renderres:(row + 1) * self.renderres, col * self.renderres:(col + 1) * self.renderres,
                :] += self.BIGSPRITES[AGENT]
            w, h, c = img.shape
    ...
    def pretty_render(self, mode='human'):
        ...
        if mode == 'human':
            ...
            if len(self.state[HOLDING]) > 0:
                ...
                img[w - self.pretty_render_res:w, self.pretty_render_res:self.pretty_render_res * 2] = \
                self.pretty_render_sprites[root]
            # cv2.imwrite(RENDER_DIR+'pretty_img{:04d}_{:04d}.png'.format(self.episode, self.state['count']), img)
            # return img
            plt.imshow(img.reshape(33 * 10, 30 * 10, 3)[:, :, ::-1])
            plt.show()
            return img
